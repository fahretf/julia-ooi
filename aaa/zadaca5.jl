...
function dodajFakeVarijable(ST, csigns)
  for i = 1:size(csigns)[1]
...
function kreirajPocetnuTabelu(goal::String, A, b::Vector, c::Vector, csigns::Vector, vsigns::Vector)
  ST = A
  ST = dodajFakeVarijable(ST, csigns)
...
function kreirajPocetnuBazu(ST::Matrix, b::Vector, c::Vector, csigns::Vector)
  baza = zeros(size(b)[1])
  for i = 1:size(csigns)[1]
...
function imaVjestackih(baza, csigns, c)
...
function dajVodecuKolonuSaM(ST, b)
  koeficijentUzMMax = -1
  vodećaKolona = -1
...
function dajVodeciRedSaM(ST, vodećaKolona)
  tMax = Inf
  vodećiRed = -1
...
function transformisiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
...
function izvrsiA(ST, baza, csigns::Vector, c::Vector, b::Vector)
  while imaVjestackih(baza, csigns, c)
    vodećaKolona = dajVodecuKolonuSaM(ST, b)
    if vodećaKolona == -1
      throw(error("Nema rješenja!"))
    end
    vodećiRed = dajVodeciRedSaM(ST, vodećaKolona)
    baza[vodećiRed] = vodećaKolona 
    if (!imaLiVještačkih(baza, csigns, c))
      ST = [ST[1:(size(ST)[1] - 2), :]; transpose(ST[size(ST)[1], :])]
      ST = transformisiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
      break;
    end
    ST = transformisiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
  end
...
function bIzvrsi(ST, baza, b::Vector, c::Vector, csigns::Vector)  
...
function dajRjesenja(ST, baza, goal::String)
...
function izrsiSimpleks(ST, baza, b::Vector, csigns::Vector, c::Vector, goal) 
  try 
    ST, baza = izvrsiA(ST, baza, csigns, c, b)
    ST, baza = bIzvrsi(ST, baza, b::Vector, c::Vector, csigns)
  catch izuzetak
    throw(error(izuzetak.msg))
  end
  optimum, optimalnaVrijednost = dajRjesenja(ST, baza, goal) 
...
function dajIzravnavajuce(optimumTacke2, csigns2, vsigns2, c)
  izravnavajuce = []
  vjestacke = []
  iterator = size(c)[1]+1
...
function obradiRješenje(optimum, vsigns, csigns, c)
  ...
  izravnavajućeVarijable, vjestackeVarijable = dajIzravnavajuce(optimum, csigns, vsigns, c)
...
function dajVrijednostiDualnih(goalPrimal, cPrimal, APrimal, bPrimal, csignsPrimal, vsignsPrimal)
  ...
  STDual = kreirajPocetnuTabelu(goalDual, ADual, bDual, cDual, csignsDual, vsignsDual)
  bazaDual = kreirajPocetnuBazu(STDual, bDual, cDual, csignsDual)
  STDual, bazaDuaL, optimumDual, optimalnaVrijednostDual = izrsiSimpleks(STDual, bazaDual, bDual, csignsDual, cDual, goalDual)
...
function general_simplex(goal::String,c::Matrix, A::Matrix, b::Vector, csigns, vsigns::Vector)
  ...
  ST = kreirajPocetnuTabelu(goal::String, A::Matrix, b::Vector, cvektor::Vector, csigns::Vector, vsigns::Vector)
  baza = kreirajPocetnuBazu(ST, b, cvektor, csigns) 
  try 
    ST, baza, optimum, optimalnaVrijednost = izrsiSimpleks(ST, baza, b, csigns, cvektor, goal)
