...
#Zadaca5 Fahret Fijuljanin (18292)
function uporediRealne(x, y)
  epsilon = 10^(-9)
...
############### FUNKCIJA ZA VERIFIKACIJU ? ####################

#

function dajVjestackeVarijable(csigns, c)
...
function prosiriKoeficijente(A, kolonaModifikacije::Int64)
  novaMat = Matrix{Float64}(undef, size(A)[1], size(A)[2] + 1)
  for i = 1:size(A)[1]
    for j = 1:size(A)[2] + 1
      if (j == kolonaModifikacije + 1)
        novaMat[i, j] = A[i, j - 1] * (-1)
...
function uvediSmjenu(A, c::Vector, vsigns::Vector)
  ...
  for i = 1:size(vsigns)[1]
    ...
    elseif (vsigns[i] == 0)
      ...
      A = prosiriKoeficijente(A, brojVarijabli - 1)
    end
  end
  return A
end
...
function dajKoeficijenteUzVarijable(goal::String, ST::Matrix, c::Vector, b::Vector, csigns::Vector)
  ...
  for j = 1:size(koeficijenti)[2]
    if (!uporediRealne(koeficijenti[1, j], 0))
      nemaM = false
      break
    end
  end
...
function dodajDodatneIVještačkeVarijable(ST, csigns)
  for i = 1:size(csigns)[1]
...
function kreirajPočetnuTabelu(goal::String, A, b::Vector, c::Vector, csigns::Vector, vsigns::Vector)
  ST = A
  ST = dodajDodatneIVještačkeVarijable(ST, csigns)
...
function dajElementBaze(ST::Matrix, indeksOgraničenja::Int64, c::Vector)
  for j = (size(c)[1] + 1):(size(ST)[2] - 1)
    if (ST[indeksOgraničenja, j] == 1)
      bazniElement = true
      for i = 1:size(ST)[1]
        if (i != indeksOgraničenja && !uporediRealne(ST[i, j], 0))
          bazniElement = false
        end
      end
      if (bazniElement)
...
function kreirajPočetnuBazu(ST::Matrix, b::Vector, c::Vector, csigns::Vector)
  baza = zeros(size(b)[1])
  for i = 1:size(csigns)[1]
...
function imaLiVještačkih(baza, csigns, c)
  vještačkeVarijable = dajVjestackeVarijable(csigns, c) 
  for i = 1:size(baza)[1]
...
function dajVodećiRedM(ST, vodećaKolona)
  tMax = Inf
  vodećiRed = -1
...
function transformišiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
...
function izvršiDoIzbacivanjaVještačkih(ST, baza, csigns::Vector, c::Vector, b::Vector)
  while imaLiVještačkih(baza, csigns, c)
    vodećaKolona = dajVodećuKolonuM(ST, b)
    if vodećaKolona == -1
      throw(error("Nema rješenja!"))
    end
    vodećiRed = dajVodećiRedM(ST, vodećaKolona)
    baza[vodećiRed] = vodećaKolona 
    if (!imaLiVještačkih(baza, csigns, c))
      ST = [ST[1:(size(ST)[1] - 2), :]; transpose(ST[size(ST)[1], :])]
      ST = transformišiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
      break;
    end
    ST = transformišiSimpleksTabelu(ST, vodećiRed, vodećaKolona)
  end
...
function dajVodećuKolonu(ST, b, c, csigns) 
  koeficijentUFunkcijiMax = -1
  brojOgraničenja = size(b)[1]
  vodećaKolona = -1
  for j = 1:(size(ST)[2] - 1)
    if !(j in dajVjestackeVarijable(csigns, c)) &&  ST[brojOgraničenja + 1, j] > 0 && ST[brojOgraničenja + 1, j] >= koeficijentUFunkcijiMax
...
function dajVodećiRed(ST, b::Vector, vodećaKolona)
  tMax = Inf
  brojOgraničenja = size(b)[1]
  vodećiRed = -1
  for i = 1:brojOgraničenja
...
function izvršiNakonIzbacivanjaVještačkih(ST, baza, b::Vector, c::Vector, csigns::Vector)  
  optimumDostignut = false
  while !optimumDostignut
    vodećaKolona = dajVodećuKolonu(ST, b, c, csigns)
    if vodećaKolona == -1
      optimumDostignut = true
      break
    end
    vodećiRed = dajVodećiRed(ST, b, vodećaKolona)
    if vodećiRed == -1
...
function dajRješenja(ST, baza, goal::String)
...
function izvršiSimpleksAlgoritam(ST, baza, b::Vector, csigns::Vector, c::Vector, goal) 
  try 
    ST, baza = izvršiDoIzbacivanjaVještačkih(ST, baza, csigns, c, b)
    ST, baza = izvršiNakonIzbacivanjaVještačkih(ST, baza, b::Vector, c::Vector, csigns)
  catch izuzetak
    throw(error(izuzetak.msg))
  end
  optimum, optimalnaVrijednost = dajRješenja(ST, baza, goal) 
...
function dajIzravnavajućeVarijable(optimumTacke2, csigns2, vsigns2, c)
  izravnavajuce = []
  vjestacke = []
  iterator = size(c)[1]+1
...
function obradiRješenje(optimum, vsigns, csigns, c)
  izvorneVarijable = dajIzvorneVarijable(optimum, vsigns)
  izravnavajućeVarijable, vjestackeVarijable = dajIzravnavajućeVarijable(optimum, csigns, vsigns, c)
  return izvorneVarijable, izravnavajućeVarijable, vjestackeVarijable
end

function jeLiDegenerirano(tabela)
  for i = 1:(size(tabela)[1] - 1)
    if (uporediRealne(tabela[i, size(tabela)[2]], 0))
      return true
    end
  end
  return false
...
function jeLiJedinstveno(tabelaST, baza1, csigns, c)
  ...
  for i=1:brojKolona
    if(uBazi(i, dajVjestackeVarijable(csigns, c))==false)
...
function dajVrijednostiDualnih(goalPrimal, cPrimal, APrimal, bPrimal, csignsPrimal, vsignsPrimal)
  ...
  STDual = kreirajPočetnuTabelu(goalDual, ADual, bDual, cDual, csignsDual, vsignsDual)
  bazaDual = kreirajPočetnuBazu(STDual, bDual, cDual, csignsDual)
  STDual, bazaDuaL, optimumDual, optimalnaVrijednostDual = izvršiSimpleksAlgoritam(STDual, bazaDual, bDual, csignsDual, cDual, goalDual)
  izvorneVarijable, izravnavajućeVarijable, vještačkeVarijable = obradiRješenje(optimumDual, vsignsDual, csignsDual, cDual)
...
function general_simplex(goal::String,c::Matrix, A::Matrix, b::Vector, csigns, vsigns::Vector)
  ...
  ST = kreirajPočetnuTabelu(goal::String, A::Matrix, b::Vector, cvektor::Vector, csigns::Vector, vsigns::Vector)
  baza = kreirajPočetnuBazu(ST, b, cvektor, csigns) 
  try 
    ST, baza, optimum, optimalnaVrijednost = izvršiSimpleksAlgoritam(ST, baza, b, csigns, cvektor, goal)
    izvorneVarijable, izravnavajuceVarijable, vjestackeVarijable = obradiRješenje(optimum, vsigns, csigns, cvektor)
